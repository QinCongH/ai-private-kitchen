...
class KitchenService:
    ...
    def _build_human_message(self, message: str, image_url: str | None = None) -> HumanMessage:
        if image_url:
            return HumanMessage(
                content=[
    ...
    def stream_chat(self, message: str, thread_id: str, image_url: str | None = None):
        """流式聊天，yield SSE 格式数据"""
        human_msg = self._build_human_message(message, image_url)
        ...
        for token, metadata in self.agent.stream(
            ...
            if token.content and isinstance(token.content, str):
                yield f"data: {json.dumps({'content': token.content}, ensure_ascii=False)}\n\n"
